...
const normalizeCategoryKey = (value?: string | null) => {
  const normalized = normalizeMainCategory(value);
...
const formatTireSize = (tireWidth?: string, tireHeight?: string, tireDiameter?: string) => {
  ...
  const tireSizeDiameter = toText(tireDiameter);
  if (width && height && tireSizeDiameter) return `${width}/${height} R${tireSizeDiameter}`;
  if (tireSizeDiameter) return `R${tireSizeDiameter}`;
  return '';
};
...
const TechnicalDataSection: React.FC<TechnicalDataSectionProps> = ({
  ...
  switch (normalizedMainCategory) {
    case 'parts':
      ...
      addItem('condition', 'Състояние', formatConditionLabel(condition), ShieldCheck);
      break;
    case 'wheels':
      addItem(
        'offer-type',
        'Тип оферта',
        WHEEL_OFFER_TYPE_LABELS[toText(offerType)] || offerType,
        PackageOpen
      );
    ...
    case 'trucks':
      ...
      addItem(
        'euro',
        'Евро стандарт',
        formatEuroStandardLabel(heavyEuroStandard || euroStandard),
        Leaf
      );
    ...
    default:
      addItem('date', 'Дата на производство', formatDate(year, month), Calendar);
      addItem('fuel', 'Гориво', formatFuelLabel(fuel), Fuel);
      addNumericItem('power', 'Мощност', power, 'к.с.', Gauge);
      addNumericItem('displacement', 'Кубатура', displacement, 'см³', Ruler);
      addItem('gearbox', 'Скоростна кутия', formatGearboxLabel(gearbox), Cog);
      ...
      addItem('condition', 'Състояние', formatConditionLabel(condition), ShieldCheck);
      addItem('euro', 'Евро стандарт', formatEuroStandardLabel(euroStandard), Leaf);
      break;
  }
  ...
  if (isCarCategory && items.length < 3) addItem('fallback-fuel', 'Гориво', formatFuelLabel(fuel), Fuel);
  if (isCarCategory && items.length < 3) {
    addItem('fallback-gearbox', 'Скоростна кутия', formatGearboxLabel(gearbox), Cog);
  }
  if (items.length < 3) addItem('fallback-city', 'Град', city, Route);
  if (items.length < 3) addItem('fallback-condition', 'Състояние', formatConditionLabel(condition), ShieldCheck);
